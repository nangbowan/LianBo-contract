...
export abstract class DRC721_draft extends IDRC721 {
  ...
  protected replaceIssue(owner: string, assetStr: string): void {
    ...
    const assetObj = this.parseAssetWithId(assetStr);

    //这里修改了assembly/deps/json 中的返回值
    //getWithoutNULL,只允许确定有返回值的情况
    //不够严谨，后续完善
    const assetId = assetObj.getWithoutNULL("assetId").toString();
  ...
  /**
   * 根据 JSON string 解析出对象，已经储存的资产才可调用此方法
   * @param assetStr AssetDetail JSON string
   * @returns AssetDetail object
   */
  protected parseAssetWithId(assetStr: string): JSON.Obj {
    const assetObj = JSON.parse(assetStr) as JSON.Obj;

    // 检查字段
    const assetInfo = assetObj.getValue('assetInfo');
    const assetUri = assetObj.getValue('assetUri');
    const assetId = assetObj.getValue('assetId');
    assert(
      assetInfo != null || assetUri != null,
      '[DRC721] parseAssetWithId: asset must has assetInfo or assetUri'
    );

    assert(assetId != null, '[DRC721] parseAssetWithId: asset must has assetId')
